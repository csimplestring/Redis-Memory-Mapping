...
void zaddGenericCommand(redisClient *c, int incr) {
    ...
    if (zobj == NULL) {
        ...
        dbAdd(c->db,key,zobj);	//hook !
    } else {
    	// convert vmpointer to zset object
    	if(server.mmap_enabled){
    	    		vmpointer *vp = (vmpointer*)zobj;
    	    		zobj = mmapReadObject(vp,0);
    	    	}
    ...
    // after zset is added into memory, swap it into mmap-file
    if(server.mmap_enabled){
    	  vmpointer *vp ;
    	   robj *v;
    	   if( (vp = mmapSwapObjectBlocking(zobj)) == NULL)
    	   {
    		addReplyError(c, "can't swap hash in to mmaped file");
    		return;
    	   }
    	   key = c->argv[1];
    	   (dictFind(c->db->dict, key->ptr))->val = vp;
    }
...
void genericZrangebyscoreCommand(redisClient *c, int reverse, int justcount) {
    ...
    if ((zobj = lookupKeyReadOrReply(c,key,emptyreply)) == NULL)  return;

    // recover zset object
    if(server.mmap_enabled){
    	vmpointer *vp = (vmpointer*)zobj;
    	zobj = mmapReadObject(vp, 1);
    }
