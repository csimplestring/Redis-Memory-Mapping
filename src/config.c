...
void loadServerConfig(char *filename) {
    ...
    while(fgets(buf,REDIS_CONFIGLINE_MAX+1,fp) != NULL) {
        ...
        if (!strcasecmp(argv[0],"mmap_enable") && argc == 2)
        {
        	server.mmap_enabled = atoi(argv[1]);
		}
        else if (!strcasecmp(argv[0],"mmap_pages") && argc == 2)
               {
               	server.mmap_pages = atoll(argv[1]);
       		}
        else if (!strcasecmp(argv[0],"mmap_page_size") && argc == 2)
               {
               	server.mmap_page_size = atoi(argv[1]);
       		}
        else if (!strcasecmp(argv[0],"mmap_file_name") && argc == 2)
               {
        		server.mmap_file_name = sdsnew(argv[1]);
               	strcpy(server.mmap_file_name,  argv[1]);
       		}
