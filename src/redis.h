...
typedef struct mmap_ptr
{
	char *current_ptr;
	char *start_ptr;
	char *mmap_file_name;
	off_t mmap_file_size;
    int mmap_file_fd;
}mmap_ptr;
...
struct redisServer {
    ...
    /* mmap file operaton */
    int mmap_enabled;
    mmap_ptr * mmap_poniter;
    int mmap_page_size;
    off_t mmap_pages;
    off_t mmap_next_page; /* Next probably empty page */
	off_t mmap_near_pages; /* Number of pages allocated sequentially */
	unsigned char *mmap_bitmap; /* Bitmap of free/used pages */
	char *mmap_file_name;

	unsigned long long mmap_stats_used_pages;
	unsigned long long mmap_stats_swapped_objects;
	unsigned long long mmap_stats_swapouts;
	unsigned long long mmap_stats_swapins;
